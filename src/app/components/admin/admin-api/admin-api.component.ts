...
export class AdminApiComponent implements OnInit {
  ...
  private buildDoc(): void {
    ...
    const lines = [
      ...
      '| `language` | string | Language code, e.g. `ca`, `es`, `en`. Defaults to `ca`. |',
      '| `translationOfId` | number | Link this post as a translation of another (see below). |',
      '| `published` | boolean | Draft when false. |',
      '| `publishedAt` | string | `YYYY-MM-DD` or ISO. Empty → today when publishing. A **future** date schedules the post (see below). |',
      '',
      '### Multilingual posts',
      '',
